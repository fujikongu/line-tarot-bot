
from flask import Flask, request, abort
from linebot import LineBotApi, WebhookHandler
from linebot.exceptions import InvalidSignatureError
from linebot.models import MessageEvent, TextMessage, TextSendMessage, QuickReply, QuickReplyButton, MessageAction

app = Flask(__name__)

# 各種キーを環境変数または設定ファイルなどで管理することを推奨
line_bot_api = LineBotApi('YOUR_CHANNEL_ACCESS_TOKEN')
handler = WebhookHandler('YOUR_CHANNEL_SECRET')

# 認証済みパス（noteなどで告知している共通パス）
VALID_PASSWORD = "mem1091"

# パス認証状態を保存する簡易データベース（本番はDB推奨）
authenticated_users = set()

@app.route("/callback", methods=['POST'])
def callback():
    signature = request.headers['X-Line-Signature']
    body = request.get_data(as_text=True)
    try:
        handler.handle(body, signature)
    except InvalidSignatureError:
        abort(400)
    return 'OK'

@handler.add(MessageEvent, message=TextMessage)
def handle_message(event):
    user_id = event.source.user_id
    user_text = event.message.text.strip()

    if user_text.startswith("会員パス："):
        password = user_text.replace("会員パス：", "").strip()
        if password == VALID_PASSWORD:
            authenticated_users.add(user_id)
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(
                    text="認証完了しました。占いを始められます。",
                )
            )
            return

    if user_id not in authenticated_users:
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(
                text="このBotを利用するには、noteで公開されている『会員パス』を送信してください。\n例：会員パス：123abc"
            )
        )
        return

    # 認証後の占いジャンル選択を促す
    if user_text in ["占って", "占い", "占う"]:
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(
                text="占いたい項目を教えてください。\n例：「恋愛運」「金運」「仕事運」「結婚運」「今日の運勢」",
                quick_reply=QuickReply(
                    items=[
                        QuickReplyButton(action=MessageAction(label="恋愛運", text="恋愛運")),
                        QuickReplyButton(action=MessageAction(label="金運", text="金運")),
                        QuickReplyButton(action=MessageAction(label="仕事運", text="仕事運")),
                        QuickReplyButton(action=MessageAction(label="結婚運", text="結婚運")),
                        QuickReplyButton(action=MessageAction(label="今日の運勢", text="今日の運勢")),
                    ]
                )
            )
        )
        return

    # 占い結果の仮定メッセージ
    if user_text in ["恋愛運", "金運", "仕事運", "結婚運", "今日の運勢"]:
        result = f"{user_text}の占い結果です。詳細はAIが作成する結果をここに挿入します。"
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text=result)
        )
        return

    # その他
    line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text="「占って」と入力すると占いが始まります。")
    )

if __name__ == "__main__":
    app.run()
